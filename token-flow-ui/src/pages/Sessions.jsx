...
export default function Sessions() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setSessions(await getLocalSessions())
    } catch (e) {
            ...
            <div key={s.email} style={{ ...CARD, borderColor: s.is_active ? `${ACCENT}66` : '#2a2a40', position: 'relative' }}>
              ...
              <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', marginBottom: 16 }}>
                {[['Calls', fmt(s.total_calls), '#60a5fa'],
                  ...
                  ['Cost', fmtCost(s.cost_usd), '#34d399'],
                  ['Last seen', fmtDate(s.last_seen), '#888']].map(([label, val, color]) => (
                  <div key={label}>
                    <div style={{ fontSize: 10, color: '#555', textTransform: 'uppercase', letterSpacing: 0.5 }}>{label}</div>
                    <div style={{ fontSize: label === 'Last seen' ? 13 : 18, fontWeight: 700, color, marginTop: 2 }}>{val}</div>
                  </div>
                ))}
              </div>
